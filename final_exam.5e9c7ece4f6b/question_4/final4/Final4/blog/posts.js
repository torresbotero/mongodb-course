...
function PostsDAO(db) {
    ...
    this.incrementLikes = function(permalink, comment_ordinal, callback) {
        ...
        posts.findOne({'permalink': permalink}, function(err, post) {
            ...
            if (typeof post.comments[comment_ordinal].num_likes === 'undefined') {
                post.comments[comment_ordinal].num_likes = 1;
            } else {
                var numLikes = post.comments[comment_ordinal].num_likes;
                post.comments[comment_ordinal].num_likes = numLikes + 1;
            }
